...
**Your answer:** The number of observations is `r nrow(edibnb)`.

2.  Run `View(edibnb)` in your Console to view the data in the data viewer. What does each row in the dataset represent?

**Your answer:** Each row represents one Airbnb listing in Edinburgh. It includes information such as the price, neighborhood, number of guests, bedrooms, bathrooms, rating, and number of reviews.
...
**Your answer:** I chose a binwidth of 50 because it shows enough detail to compare the price distributions without creating too many narrow bars.
...
**Your answer:** facet_wrap(~neighbourhood) I chose the regular grid because it makes the neighborhoods easier to compare without creating one very long row or column.
...
```{r final-price-histogram, eval = TRUE, fig.alt = "Faceted histogram of Airbnb price by neighborhood, using the binwidth and layout chosen above."}
ggplot(data = edibnb, mapping = aes(x = price)) +
  geom_histogram(binwidth = 50) +
  facet_wrap(~neighbourhood)
```
...
top_5 <- edibnb %>%
  ...
  slice_max(median_price, n = 5, with_ties = FALSE)
...
```{r ridge-plots, message = FALSE, eval = TRUE, fig.alt = "Ridge plot comparing the distribution of Airbnb prices across the top 5 most expensive neighborhoods."}
library(ggridges)
...
edibnb %>%
  ...
  labs(
    x = "Price",
    y = "Neighborhood",
    title = "Airbnb Price Distributions for the Top 5  Neighborhoods"
   )
```
...
**Your answer:** The neighborhood that has the highest typical prices is New Town has the highest typical prices because it has the highest median price of 100. Old Town has the most overall variation because it has the highest standard deviation, while New Town has the largest IQR. All five neighborhoods have mean prices higher than their medians, and the ridge plots show long right tails, which suggests that there are some unusually expensive listings
...
```{r review-scores-viz, eval = TRUE, fig.alt = "Visualization comparing the distribution of Airbnb review scores across neighborhoods."}
edibnb %>%
  filter(!is.na(neighbourhood)) %>%
  ggplot(aes(x = neighbourhood, y = review_scores_rating)) +
  geom_boxplot() +
  coord_flip() +
  labs(
    title = "Airbnb Review Scores by Neighborhood",
    x = "Neighborhood",
    y = "Review Score"
  )
```
...
**Your answer:** Airbnb guests generally rate the properties highly because most neighborhood ratings are between 90 and 100. The neighborhoods have similar typical ratings. Although Southside and Newington appear to have a little more variation. Several neighborhoods also have unusually low ratings that appear as outliers.
